...
func (se *statusError) GetCode() codes.Code {
	return codes.Code((*spb.Status)(se).GetCode())
}

func (se *statusError) GetMessage() string {
	return (*spb.Status)(se).GetMessage()
}
...
func (se *statusError) GetDetails() (details []interface {
	...
	for _, detail := range (*spb.Status)(se).GetDetails() {
...
func FromError(err error) (s *Status, ok bool) {
	...
	if st, ok := status.FromError(err); ok {
		newSt := New(st.Code(), st.Message())
		p := newSt.Proto()
		for _, detail := range st.Proto().GetDetails() {
			p.Details = append(p.GetDetails(), (*types.Any)(detail))
		}
		return newSt, true
	}
